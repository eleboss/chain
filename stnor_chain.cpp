  ...
  for(int i=0; i<process_data_num; i++)
  {
    ...
    { 
      ...
      if(cur_position->prev == NULL)
      {
        ...
        if(*cur_position->next->wp != *cur_position->wp)
        {
          loop_pointer = cur_position->next;
          while (loop_pointer != NULL) {  
            *loop_pointer->wp -= PER_WEIGHT;
      ...
      else
      {
        ...
        if(*cur_position->next->wp != *cur_position->wp and *cur_position->prev->wp != *cur_position->wp)
        {
          loop_pointer = cur_position->next;
          while (loop_pointer != NULL) {  
            *loop_pointer->wp -= PER_WEIGHT;
      ...
      if((cur_position->time != tail->time) )
      {
        *cur_position->wp = *tail->wp + PER_WEIGHT;
      }
      else
      {
        *cur_position->wp = *tail->wp;
      }
